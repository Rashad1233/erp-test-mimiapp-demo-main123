...
function initializeTranslations() {
  ...
  window.translations = {
    ...
    'az': {
      ...
      // Anbar hesabatı üçün tərcümələr
      'inventoryReport': 'Anbar Hesabatı',
      'productsSummary': 'Məhsulların Xülasəsi',
      'totalProducts': 'Ümumi Məhsullar',
      'lowStockItems': 'Az Ehtiyatlı Məhsullar',
      'searchProducts': 'Məhsulları axtar...',
      'noDataAvailable': 'Anbar məlumatları mövcud deyil',
      'visualizationPlaceholder': 'Vizualizasiya burada',
      'productName': 'Məhsulun Adı',
      'category': 'Kateqoriya',
      'currentStock': 'Mövcud Ehtiyat',
      'minStock': 'Min. Ehtiyat',
      'supplierName': 'Təchizatçı',
      'stockVisualization': 'Ehtiyat Vizualizasiyası',
      'tryAdjustingFilters': 'Filtrləri tənzimləyə və ya anbarınıza məhsul əlavə edə bilərsiniz',
      'inventoryDetails': 'Anbar Təfərrüatları',
      'inventoryDistribution': 'Anbar Bölgüsü',
      ...
      'audio': 'Audio',
      'dataStorage': 'Məlumat Saxlama',
      'networkEquipment': 'Şəbəkə Avadanlığı',
      'components': 'Komponentlər',
      
      // Müştəri hesabatı üçün tərcümələr
      'clientReport': 'Müştəri Hesabatı',
      'exportFullReport': 'Tam Hesabatı PDF-ə ixrac et',
      'clientsSummary': 'Müştərilərin Xülasəsi',
      'totalClients': 'Ümumi Müştərilər',
      'newClientsThisMonth': 'Bu Ay Yeni Müştərilər',
      'activeClients': 'Aktiv Müştərilər',
      'inactiveClients': 'Qeyri-aktiv Müştərilər',
      'clientsList': 'Müştərilərin Siyahısı',
      'searchClients': 'Müştəriləri axtar...',
      'lastOrder': 'Son Sifariş',
      'orderTotal': 'Sifariş Məbləği',
      'orderCount': 'Sifariş Sayı',
      'orderStatus': 'Sifariş Statusu',
      'noClientData': 'Göstəriləcək müştəri məlumatı yoxdur.',
      
      // Təchizatçı hesabatı üçün tərcümələr
      'supplierReport': 'Təchizatçı Hesabatı',
      'suppliersSummary': 'Təchizatçıların Xülasəsi',
      'totalSuppliers': 'Ümumi Təchizatçılar',
      'suppliersList': 'Təchizatçıların Siyahısı',
      'searchSuppliers': 'Təchizatçıları axtar...',
      'paymentTerms': 'Ödəniş Şərtləri',
      'noSupplierData': 'Göstəriləcək təchizatçı məlumatı yoxdur.',
      
      // Az ehtiyat hesabatı üçün tərcümələr
      'lowStockReport': 'Az Ehtiyat Hesabatı',
      'lowStockSummary': 'Az Ehtiyat Xülasəsi',
      'lowStockItemsList': 'Az Ehtiyatlı Məhsulların Siyahısı',
      'searchLowStockItems': 'Az ehtiyatlı məhsulları axtar...',
      'totalLowStockItems': 'Ümumi Az Ehtiyatlı Məhsullar',
      'noLowStockData': 'Göstəriləcək az ehtiyatlı məhsul məlumatı yoxdur.',
      
      // Maliyyə hesabatı üçün tərcümələr
      ...
      'searchTransactions': 'Əməliyyatları axtar...',
...
function renderInventoryReport(data) {
  ...
  const formatCurrency = (value) => {
    const formatter = new Intl.NumberFormat(
      ...
      currentLanguage === 'az' ? 'az-AZ' : 'en-US', 
      {
        style: 'currency',
        currency: 'USD',
...
function renderClientsReport(data) {
  ...
  const currentDate = new Date();
  const dateOptions = { year: 'numeric', month: 'short', day: 'numeric' };
  const formattedDate = currentDate.toLocaleDateString(currentLanguage === 'ru' ? 'ru-RU' : currentLanguage === 'az' ? 'az-AZ' : 'en-US', dateOptions);
...
function formatDate(dateString) {
  ...
  try {
    ...
    if (window.currentLanguage === 'ru') {
      locale = 'ru-RU';
    } else if (window.currentLanguage === 'az') {
      locale = 'az-AZ';
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Добавляем азербайджанский язык в селектор языков, если он существует
  const languageSelector = document.getElementById('language-selector');
  if (languageSelector && !languageSelector.querySelector('option[value="az"]')) {
    const azOption = document.createElement('option');
    azOption.value = 'az';
    azOption.textContent = 'Azərbaycan';
    languageSelector.appendChild(azOption);
