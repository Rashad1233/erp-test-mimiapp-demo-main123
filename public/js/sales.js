...
// Load sales data
function loadSales() {
  // Обновим заголовки таблицы в соответствии с текущим языком
  updateTableHeaders();
  ...
  fetch('/api/sales')
    ...
    .then(data => {
      ...
      if (data.sales.length === 0) {
        const noSalesText = getTranslation('noSalesFound', 'No sales found');
        tableBody.innerHTML = `<tr><td colspan="6" class="text-center">${noSalesText}</td></tr>`;
        return;
      }
      
      data.sales.forEach(sale => {
        const viewText = getTranslation('view', 'View');
        const pdfText = getTranslation('pdf', 'PDF');
        ...
        row.innerHTML = `
          ...
          <td><span class="status-badge ${sale.status.toLowerCase()}">${statusDisplay}</span></td>
          <td>
            <div class="action-buttons">
              <button class="action-btn view-btn" data-id="${sale.id}" title="${viewText}"><i class="fas fa-eye"></i></button>
              <button class="action-btn pdf-btn" data-id="${sale.id}" title="${pdfText}"><i class="fas fa-file-pdf"></i></button>
              <button class="action-btn delete-btn" data-id="${sale.id}" title="${deleteText}"><i class="fas fa-trash"></i></button>
            </div>
          </td>
        `;
    ...
    .catch(error => {
      const errorMessage = getTranslation('errorLoadingSales', 'Error loading sales:') + ' ' + error.message;
      showNotification(errorMessage, 'error');
    });
}
